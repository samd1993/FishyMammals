...
#This code includes data importation, processing, filtering procedures and exploratory analyses.
#Also includes shared ASVs analysis and plot visualizations
...
icf2
 ...
 icfp <- prune_samples(sample_sums(icf)>=500, icf)
icfpr=rarefy_even_depth(icfp)

##~~~~~~~
##  glomming at diff levels now
icg=tax_glom(icf,taxrank="Genus")
icg1=tax_glom(icf1,taxrank="Genus")
icf1=tax_glom(icf1,taxrank="Family")


icg=tax_glom(icfpr,taxrank="Genus")
...
mnhco
mnhco$variable=factor(mnhco$variable,levels=c("herbshared","carnshared"))
```

```{r}
pcolors=c("Proteobacteria"="#003f5c",
Bacteroidota="#444e86",
Verrucomicrobiota="#955196",
Spirochaetota="#dd5182",
Firmicutes="#ff6e54",
Fusobacteria="#ffa600")
```

```{r}
col=c(Firmicutes="#444e86",
Fusobacteria="#003f5c",
Proteobacteria="#ff6e54",
Firmicutes="#444e86",
Firmicutes="#444e86",
Firmicutes="#444e86",
Verrucomicrobiota="#dd5182",
Proteobacteria="#ff6e54",
Firmicutes="#444e86",
Proteobacteria="#ff6e54",
Firmicutes="#444e86",
...
(p=ggplot(mnhco,aes(otu,value,fill=variable))+
  ...
  ylim(-1000,4000)+
   ...
   ylab("Shared ASVs")+
    labs(fill = "Host diet") +
  scale_fill_manual(labels = c("herbivore", "carnivore"), values = c("#444e86", "#ff6e54"))+
    theme_classic()+
...
mkp
mkp$variable=factor(mkp$variable,level=c("herb","carn"))
```
...
(p=ggplot(mkp,aes(reorder(otu,-value),value,fill=variable))+
  ...
  ylim(-7,5.5)+
    theme_classic()+
    ylab("clr (differential abundance)")+
    labs(title = "Top herbivore and carnivore predicted pathways", fill = "Host diet") +
  scale_fill_manual(labels = c("herbivore", "carnivore"), values = c("#444e86", "#ff6e54"))+
...
~~~~~~~~~~~~~~~~~~
##Doing fish vs mammal venn count comparison


```{r}
library("limma")
fishmam = c("Actinopterygii", "Mammalia")
# Here's the recommended/intended one line
imgf=subset_samples(icg, host %in% fishmam)
GB_st = merge_samples(imgf, "host")
sample_data(GB_st)

table_GB_st <- t(otu_table(GB_st, ))
head(table_GB_st)
write.csv(table_GB_st,file="fishmam_genus.csv")
write.csv(imgf@tax_table,file="fishmam_tax.csv")
...
```{r}
fm=read.csv("fishmam-fig.csv",sep=",")
fm$Name <- factor(fm$Name, levels=unique(fm$Name))
fm$Host <- factor(fm$Host, levels=c("Mammal","Fish"))
```

```{r}
(p=ggplot(fm,aes(Name,Variable,fill=Host))+
    geom_bar(stat="identity")+
  ylim(-5000,8000)+
    theme_grey()+
   ylab("Differentially abundant ASVs")+
    labs(fill = "Host") +
  scale_fill_manual(labels = c("Mammal", "Fish"), values = c("#444e86","#ff6e54"))+
    theme_classic()+
    theme(axis.text.x = element_text(angle = 0,size=12),
              axis.text.y = element_text(size=10,face='bold'),
          axis.title.x = element_text(size=14,vjust=-0.8),
